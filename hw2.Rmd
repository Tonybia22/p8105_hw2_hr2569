...
snp_clean
```
**snp** data reflected the stock performance . The *close* variable referred to the closing values of the S&P stock index on the associated date ranging from `r snp_clean$year[1]` to `r snp_clean$year[length(snp_clean$year)]`
...
unemployment_clean
```
**unemployment** data described the unemployment rate over time randing from `r unemployment_clean$year[1]` to `r unemployment_clean$year[length(unemployment_clean$year)]`.
...
baseline_fem <- baseline[baseline$sex=="Female",]

print(baseline)
```

In total `r nrow(baseline)` participants who satisfied the standard were recruited. Of these participants, `r nrow(baseline[baseline$age_at_onset>0,])` subjects developed MLC. The average baselin age was `r mean(baseline$current_age)`. The portion of women who were APOE4 carrier was `r nrow(baseline_fem[baseline_fem$apoe4=="APOE4_carrier",])/nrow(baseline_fem)`.
...
The merged dataset had `r nrow(AD_merge)` observations. With `r nrow(AD_merge[AD_merge$sex=="Female",])` females and  `r nrow(AD_merge[AD_merge$sex=="Male",])` males in the dataset. The portion of participants who developed MLC in this dataset is `r nrow(AD_merge[AD_merge$age_at_onset>0,])/nrow(AD_merge)`.
